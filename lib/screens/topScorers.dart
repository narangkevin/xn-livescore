...
Future<List<topScorer>> getScorers(String code) async {
  Response r = await get(
      Uri.encodeFull(
          'https://api.football-data.org/v2/competitions/$code/scorers'),
      headers: {'X-Auth-Token': token});
  Map<String, dynamic> x = jsonDecode(r.body);
  List y = x['scorers'];
  List<topScorer> topScorersList = [];
  for (var i in y) {
    topScorersList.add(new topScorer(
        i['player']['name'], i['team']['name'], i['numberOfGoals'].toString()));
  }
...
Widget rankingRow(
    ...
    String name = 'Player',
    String team = 'Team',
    String goalNum = 'Goals']) {
  return Row(
    children: <Widget>[
      Expanded(child: Center(child: Text(pos, style: plStyle)), flex: 1),
      Expanded(
          child: Padding(
            child: Text(name, style: plStyle),
            padding: EdgeInsets.fromLTRB(5, 0, 5, 0),
          ),
          flex: 4),
      Expanded(
          child: Padding(
            child: Text(team, style: plStyle),
            padding: EdgeInsets.fromLTRB(5, 0, 5, 0),
          ),
          flex: 4),
      Expanded(
        child: Text(goalNum, style: plStyle),
        flex: 2,
      )
    ],
  );
